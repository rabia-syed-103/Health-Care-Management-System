  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        setPrescriptions(res.data.pending_prescriptions || [])
      } catch {
            ...
            <div key={i} className="card">
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{rx.prescription_date}</p>
                </div>
                <span className="badge-warning">Pending</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                <div><span className="text-gray-500">Patient:</span> <strong>{rx.patient_name}</strong></div>
                <div><span className="text-gray-500">Email:</span> <strong>{rx.patient_email}</strong></div>
                <div><span className="text-gray-500">Doctor:</span> <strong>{rx.doctor_name}</strong></div>
                <div><span className="text-gray-500">Medicines:</span> <strong>{rx.total_medicines}</strong></div>
              </div>
              <div>
