...
DROP FUNCTION IF EXISTS fn_er_doctor_before();
...
DROP INDEX IF EXISTS idx_er_doctor_availability;
...
-- Prevents scheduling ER shifts in the past and limits doctor assignments
-- to maximum 5 per shift.
...
-- Shows medicines expiring within 30 days or out of stock with alert types and
-- days until expiry for restocking and disposal decisions.
