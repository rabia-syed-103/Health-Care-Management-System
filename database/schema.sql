...
BEGIN
    ...
    IF v_bgr != v_patient_bgr THEN
        RAISE EXCEPTION
            '[Blood Inventory] Blood type mismatch for Request ID %. '
            'Patient "%" requires % but Blood ID % is %.',
            NEW.Request_ID, v_patient_name, v_patient_bgr, NEW.Blood_ID, v_bgr;
    END IF;
